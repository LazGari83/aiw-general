...
exercise_1_1 = customers_df.filter(col("city") == "New York") \
...
exercise_1_2 = customers_df.join(orders_df, "customer_id", "inner") \
    .groupBy("customer_id", "customer_name") \
    .agg(F.sum("order_amount").alias("total_amount"), 
         F.count("order_id").alias("order_count")) \
    .orderBy(F.desc("total_amount"))

print("Exercise 1.2 - Total amount per customer:")
exercise_1_2.show()
...
exercise_1_3 = customers_df.join(orders_df, "customer_id", "inner") \
    .filter(col("order_amount") > 200) \
    .select("customer_name", "email", "order_id", "order_amount") \
    .orderBy(desc("order_amount"))

print("Exercise 1.3 - Customers with orders > $200:")
exercise_1_3.show()
...
exercise_2_1 = customers_df.join(orders_df, "customer_id", "anti")

print("Exercise 2.1 - Customers with no orders:")
exercise_2_1.show()
...
exercise_2_2 = orders_df.join(customers_df, "customer_id", "anti")

print("Exercise 2.2 - Orders without customers:")
exercise_2_2.show()
...
exercise_2_3 = customers_df.join(orders_df, "customer_id", "left") \
    .groupBy("customer_id", "customer_name", "email", "city") \
    .agg(
        F.count("order_id").alias("order_count"),
        F.coalesce(F.sum("order_amount"), F.lit(0)).alias("total_amount"),
        F.coalesce(F.avg("order_amount"), F.lit(0)).alias("avg_order_amount"),
        F.coalesce(F.max("order_amount"), F.lit(0)).alias("max_order_amount")
    ) \
    .orderBy("customer_id")

print("Exercise 2.3 - Comprehensive customer report:")
exercise_2_3.show()
