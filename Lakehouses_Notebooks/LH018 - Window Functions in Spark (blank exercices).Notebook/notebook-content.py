...
window_by_top2 = Window.partitionBy("region").orderBy(desc("amount"))

sales_top_rank = sales_df.withColumn(
    "total_best_rank",
    row_number().over(window_by_top2)
)

sales_top_rank.filter(col("total_best_rank") <= 2) \
    .orderBy("region", "total_best_rank") \
    .show()
